...
def plot(x, y, z):
    ...
    # Color Depending on Z-Value
    possible_labels = list(set(z))
    a = 0
    while a < len(possible_labels):
        i = 0
        x_new, y_new, z_new = [], [], []
        while i < len(z):
            if z[i] == possible_labels[a]:
                x_new.append(x[i])
                y_new.append(y[i])
                z_new.append(z[i])
            i += 1
        # Progress
        print(int(round((possible_labels.index(z_new[0]) / len(possible_labels)) * 100)), '% Complete')
        plt.scatter(x_new, y_new, label=z_new[0], s=50, marker='o' if possible_labels.index(z_new[0]) < 10 else "x")
